...
<div class="list">
	...
	<table width=100%   id=customers >
		...
		<c:forEach items="${lendpaginationSupport.items}" var="lend">
			<tr align="center">
				...
				<td>
					<c:out value="${lend.lendState eq 2 ? '申请还书': ''}" />
				</td>
				<td>
	...
	<c:if test="${lendpaginationSupport.previousPage}">
		<a href="<c:url value="/manager/manageback?pageNo=${lendpaginationSupport.currentPageNo-1}" />" >上一页</a>
	</c:if>
	<c:if test="${userpaginationSupport.nextPage}">
		<a href="<c:url value="/manager/manageback?pageNo=${lendpaginationSupport.currentPageNo+1}" />" >下一页</a>
	</c:if>
</div> 
</body>
</html>